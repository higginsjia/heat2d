...
int main(int argc, char* argv[]) {
	...
	int numTasks, procs, matrixSize, id, id2d, steps, neighbors[4], size;
	MPI_Comm cartComm;
	...
	/*Get the work done.*/
	simulation(buffs, neighbors, chart, size, steps, cartComm,
...
void simulation(Buffer* buffs, int* neighbors, Chart chart, int size, int steps, MPI_Comm cartComm,
	...
	for (i = 0; i < steps; i++) {
		...
		/*compute borders*/
		outerUpdate(chart, size, iz, buffs, neighbors);
